...
import { buildTrendKey } from '@/lib/cache-keys';
...
export async function GET(request: Request) {
    ...
    const cacheKey = buildTrendKey(companyId, startDate, endDate);
    ...
    try {
        ...
        timer.mark('cache');
        timer.end('GET /api/marketing/trend', cacheKey, { cache: 'hit', rows: result.data.length });

        return NextResponse.json(result, { headers: CACHE_HEADERS });
    } catch (error) {
        console.error('Marketing trend API error:', error);
